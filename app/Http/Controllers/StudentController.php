...
class StudentController extends Controller {
    ...
    public function show(Request $request) {
        ...
        $sql = $this->student->builder()->where(function ($query) {
            $query->where('application_type', 'offline')
                ...
                ->orWhere(function ($q) {
                    $q->where('application_status', 1); // Only online applications with status 1
                });
        })
        ...
        foreach ($res as $row) {
            ...
            if (!$request->show_deactive) {
                ...
                // Add ID Card Preview button
                $operate .= BootstrapTableService::button('fa fa-id-card', 'javascript:void(0)', ['btn-gradient-primary', 'preview-id-card'], ['title' => __('Preview ID Card'), 'data-student-id' => $row->id, 'data-student-name' => $row->user->first_name . ' ' . $row->user->last_name, 'data-gr-number' => $row->admission_no, 'data-class' => $row->class_section->class->name . ' ' . $row->class_section->section->name]);
            } else {
    ...
    public function generate_id_card(Request $request) {
        ...
        try {
            ...
            $students = $this->user->builder()->select('id','first_name','last_name','image','school_id','gender','dob')->with('student:id,user_id,class_section_id,school_id,guardian_id,roll_number,admission_no','student.class_section.class','student.class_section.section','student.class_section.medium','student.class_section.class.stream','student.guardian:id,mobile,first_name,last_name')->whereHas('student',function($q) use($user_ids) {
